...
class ListItem extends Component {
	updateReservation = () => {
		const { firebase, item, auth } = this.props;
		const uid = auth.uid;

		firebase
			.database()
			.ref(`/Reservations/${uid}/${item[0]}`)
			.update(
				{
					isActive: false,
					isComplete: true,
			...
			<Card containerStyle={{ elevation: 3, borderRadius: 5, borderColor: '#c7c9cc', borderWidth: 1 }}>
				<View>
					...
					<View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 5 }}>
						...
						>
							Time: {item[1].pickuptime.slice(0, 21)}{' '}
						</Text>
					</View>
					<View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 5 }}>
						...
						>
							Pickup Point: {item[1].pickupPoint.description}
						</Text>
					</View>
					<View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 5 }}>
						...
						>
							Dropoff Point: {item[1].dropoffPoint.description}
						</Text>
					</View>
				</View>
				...
				{item[1].isActive && (
					<View style={{ paddingTop: 10 }}>
						<Button
							icon={{ name: 'check-circle', color: '#fff', type: 'font-awesome' }}
							onPress={this.updateReservation}
							small
							title="Mark Completed"
							fontSize={14}
							buttonStyle={{ backgroundColor: '#4e894e' }}
							borderRadius={4}
						/>
					</View>
				)}
